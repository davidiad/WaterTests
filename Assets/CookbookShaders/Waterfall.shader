...
Shader "CookbookShaders/Waterfall" { 
...
Properties {
	...
	_BaseColor ("Base color", COLOR)  = ( .184, .195, .199, 0.9)	
...
//#pragma debug
	...
	struct v2f 
	{
		...
		float4 localPos : TEXCOORD6; // use for color info
	};
	...
	v2f vert(appdata_full v, float4 vertexPos : POSITION)
	{
		v2f o;

		//** get local object position for use with color and bump and wave intensity maps *************
		//o.localPos =  mul (UNITY_MATRIX_MVP, vertexPos); // get local pos before any changes to vertex
		o.localPos =  v.vertex;// + float4(42.0, 0.0, 42.0, 0.0); // translate over by half the size of the water object (currently ~ 84)
		//********************************

		half3 worldSpaceVertex = v.vertex;//mul(_Object2World,(v.vertex)).xyz;
		half3 vtxForAni = v.vertex;//(worldSpaceVertex).xzz * 1.0; 		

		half3 nrml;
		half3 offsets;
		float4 newFrequency;
		float4 freqScale = (0.37, 1, 1.12, .853);
		//fixed4 amplitude = 0.05;//(tex2Dlod(_GMap, half4(worldSpaceVertex.xy / 90, 0.0, 0.0))) * 2.88;
//		offsets.y = 0.01;
//		offsets = (0.0, 0.0, 0.0);

		//float shift = 0.25 * (sin(_Time * freqScale.x) + 1);
//		float shift = 0.05 * ((_SinTime.x * freqScale.x) + 1);
		float4 shift = 0.5 * ((_SinTime.x * freqScale) + 1);



//		newFrequency = _Time;
//		float4 timeFactor = modf( newFrequency , out ip );
		newFrequency = float4((1.87 * shift.x) + _GFrequency.x, shift.y + _GFrequency.y, _GFrequency.z, (.87 * shift.w) + _GFrequency.w);
		...
		//float2 flowmap = (tex2D(_FlowMap, (VERTEX_WORLD_POSITION / 90.0f).xz).rg - 0.5) * 2;

		//offsets.y = 0.0; // try setting offset y to 0 -- working

		// try using a texture map to set the offsets here
//		offsets *= (tex2Dlod(_GMap, half4(worldSpaceVertex.xy / 84.0, 1.0, 1.0)));
		offsets *= (tex2Dlod(_GMap, half4(v.vertex.xz / 84.0, 1.0, 1.0)));
		//offsets *= (tex2Dlod(_GMap, half4(v.vertex.x / 84.0, 1.0, v.vertex.z / 84.0, 1.0))) * 0.5;
		...
		// false color checking of output
		//o.localPos = float4(offsets.x * 12,0,0, 0);//float4(.0, .0, 0.0,1);
		//o.localPos = float4(offsets.x * 6,0,0, 0);//float4(.0, .0, 0.0,1);

		//***********(*^%&^$% Changing the initial values of localPos
		//o.localPos = float4(offsets.x,0, offsets.z, 0);//float4(.0, .0, 0.0,1);
		//o.localPos =  float4(nrml.x,0, nrml.z, 0) * 0.5 + 0.5;
		//o.localPos = (float4(0, 0, 2 - offsets.y - v.normal.y, 1) ) ;


		//o.localPos = float4(offsets.xyz, 0.5);
	...
	half4 frag( v2f i ) : SV_Target
	{
		...
		// --------------------------- ADJUSTMENTS TO FLOW MAP SUPPORT ---------------------------
		//float2 flowmap = (tex2D(_FlowMap, (VERTEX_WORLD_POSITION / TerrainSize).xz).rg - 0.5) * 2;
		//float2 flowmap = (tex2D(_FlowMap, (VERTEX_WORLD_POSITION / 84.0f).xz).rg - 0.5) * 2;
		// replacing VERTEX_WORLD_POSITION with i.localPos so that the map is in object space
		float2 flowmap = (tex2D(_FlowMap, (i.localPos / 84.0f).xz).rg - 0.5) * 2;
		// ------------------------------------------------------------------------
		...
		half3 worldNormal = normalize(VERTEX_WORLD_NORMAL); //normalize(worldNormalLarge + worldNormalSmall); //********change here
		// ------------------------------------------------------------------------
		...
		// --------------------------- COLOR MAP SUPPORT ---------------------------
		half4 colormap = (tex2D(_CMap, (i.localPos / 84.0f).xz).rgba - 0.5) * 2;

		//half4 colormap = (tex2D(_CMap, (VERTEX_WORLD_POSITION / 84.0f).xz).rgba - 0.5) * 2.0;

		// ------------------------------------------------------------------------

		// base, depth & reflection colors
		half4 baseColor = ExtinctColor (colormap, i.viewInterpolator.w * _InvFadeParemeter.w);
		...
		baseColor.a = 1;//transparency;

		// false color check
//		baseColor.r += i.localPos.y * 0.3;
//		baseColor.g += i.localPos.y * 0.3;
//		baseColor.b += i.localPos.y * 0.4;
//		baseColor.a = 1.0;
//
		return baseColor;
	...
	{		
		...
		// test to remove reflections for now
...
Subshader 
{ 	
	Tags {"RenderType"="Transparent-200" "Queue"="Transparent"}
